...
import { EMBEDDING_MODELS, validateEmbedding, validateModelCompatibility, DEFAULT_HNSW_CONFIG } from './types/MeaningTypes.js';
...
export class MeaningDimension {
    ...
    constructor(config) {
        ...
        this.index = new HNSWIndex(this.dimensions, this.metric, config.hnswConfig ?? DEFAULT_HNSW_CONFIG);
    }
    /**
     ...
     */
    async indexEmbedding(objectHash, embedding, sourceText) {
        if (!this.initialized) {
            throw new Error('MeaningDimension not initialized. Call init() first.');
        }
        // Validate embedding
        ...
        const meaningNodeHash = nodeResult.hash;
     ...
     */
    async indexText(objectHash, text) {
        if (!this.embeddingProvider) {
            throw new Error('No embeddingProvider configured. Use indexEmbedding() with pre-computed embedding, ' +
     ...
     */
    async query(criterion) {
        if (!this.initialized) {
            throw new Error('MeaningDimension not initialized. Call init() first.');
        }
        validateEmbedding(criterion.embedding, this.dimensions);
        const results = this.index.search(criterion.embedding, criterion.k, criterion.threshold);
        return results.map(r => r.objectHash);
    }
    /**
     * Query with full results including similarity scores
     */
    async queryWithScores(criterion) {
        if (!this.initialized) {
            throw new Error('MeaningDimension not initialized. Call init() first.');
        }
        validateEmbedding(criterion.embedding, this.dimensions);
        return this.index.search(criterion.embedding, criterion.k, criterion.threshold);
    }
    /**
     * Query by text (requires embeddingProvider)
     */
    async queryByText(text, k, threshold) {
        if (!this.embeddingProvider) {
            throw new Error('No embeddingProvider configured. Use query() with pre-computed embedding, ' +
                'or provide an embeddingProvider in constructor.');
        }
        const embedding = await this.embeddingProvider.embed(text);
    ...
    async rebuildIndex() {
        try {
            ...
            const dimValues = await getAllEntries(this.dimensionHash, 'MeaningDimensionValue');
            for (const dimValueHash of dimValues) {
                try {
                    const dimValue = await getObject(dimValueHash);
                    if (!dimValue)
                        continue;
                    const meaningNode = await getObject(dimValue.meaningNodeHash);
                    if (!meaningNode)
                        continue;
                    // Validate model compatibility
                    if (meaningNode.model !== this.model) {
                        console.warn(`Skipping MeaningNode with incompatible model: ${meaningNode.model} vs ${this.model}`);
                        continue;
                    }
                    ...
                    for (const cubeObjectHash of cubeObjects) {
                        this.index.add(cubeObjectHash, dimValue.meaningNodeHash, meaningNode.embedding);
                    }
                }
                catch (err) {
...
//# sourceMappingURL=MeaningDimension.js.map